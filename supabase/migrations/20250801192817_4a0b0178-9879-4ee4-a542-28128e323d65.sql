...
-- Log dos logins criados
INSERT INTO public.logs_acesso (acao, tabela_afetada, detalhes)
SELECT 
    'LOGIN_MEDICO_CRIADO_RETROATIVO',
    'medicos_login',
    jsonb_build_object(
        'medico_id', m.id,
        'medico_nome', m.nome_completo,
        'clinica_id', m.clinica_id,
        'senha_gerada', LOWER(REPLACE(m.nome_completo, ' ', '')) || RIGHT(m.cpf, 4)
    )
FROM public.medicos m
LEFT JOIN public.medicos_login ml ON m.id = ml.medico_id
WHERE ml.medico_id IS NULL
...
WHERE id NOT IN (
    ...
    GROUP BY tipo_plano, periodo_meses
);
