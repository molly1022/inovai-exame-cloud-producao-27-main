...
-- Primeiro, atualizar/inserir a clínica
INSERT INTO public.clinicas (nome, email, telefone, subdominio, created_at, updated_at)
VALUES (
  'Integrammedica',
...
-- Obter o ID da clínica criada/atualizada
DO $$
DECLARE
  integra_clinica_id UUID;
BEGIN
  -- Buscar ID da clínica
  SELECT id INTO integra_clinica_id FROM public.clinicas WHERE email = 'contato@integrammedica.com';
  ...
  IF NOT EXISTS (SELECT 1 FROM public.assinaturas WHERE clinica_id = integra_clinica_id) THEN
  ...
  ELSE
    ...
    SET 
      ...
      limite_medicos = 15,
      updated_at = now()
    WHERE clinica_id = integra_clinica_id;
  END IF;
