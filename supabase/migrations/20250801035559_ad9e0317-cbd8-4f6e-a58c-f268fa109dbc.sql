...
-- Inserir novos planos baseados no modelo por médico (sem valor_final que é gerado)
...
('basico_medico', 1, 125.00, 125.00, 'por_medico', true, 0, 0.00,
 ARRAY['relatorios_avancados', 'integracao_whatsapp', 'backup_automatico'], true),
('basico_medico', 3, 125.00, 112.50, 'por_medico', true, 0, 10.00,
 ARRAY['relatorios_avancados', 'integracao_whatsapp', 'backup_automatico'], true),
('basico_medico', 12, 125.00, 100.00, 'por_medico', true, 0, 20.00,
 ARRAY['relatorios_avancados', 'integracao_whatsapp', 'backup_automatico'], true),

-- Plano Intermediário Por Médico  
('intermediario_medico', 1, 190.00, 190.00, 'por_medico', true, 0, 0.00,
 ARRAY['integracao_whatsapp', 'backup_automatico'], true),
('intermediario_medico', 3, 190.00, 171.00, 'por_medico', true, 0, 10.00,
 ARRAY['integracao_whatsapp', 'backup_automatico'], true), 
('intermediario_medico', 12, 190.00, 152.00, 'por_medico', true, 0, 20.00,
 ARRAY['integracao_whatsapp', 'backup_automatico'], true),

-- Plano Avançado Por Médico
('avancado_medico', 1, 299.00, 299.00, 'por_medico', true, 0, 0.00,
 ARRAY[]::text[], true),
('avancado_medico', 3, 299.00, 269.10, 'por_medico', true, 0, 10.00,
 ARRAY[]::text[], true),
('avancado_medico', 12, 299.00, 239.20, 'por_medico', true, 0, 20.00,
 ARRAY[]::text[], true);
...
BEGIN
  ...
  ELSE
    -- Modelo antigo (fixo)
    valor_por_medico := COALESCE(plano_atual.valor_final, plano_atual.valor_base);
    valor_total := COALESCE(plano_atual.valor_final, plano_atual.valor_base);
  END IF;
